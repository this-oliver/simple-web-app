...
function initApp(logfile: string): Express {
  ...
  app.get("/", (_req, res) => {
    ...
    interface LogEntry { ts: string, path: string, ua: string }
    ...
    const rows: Array<{ srcIp: string, path: string, visitCount: number, uas: string, firstTs: string, lastTs: string, timesList: string }> = [];
    for (const [srcIp, entries] of groups) {
      ...
      rows.push({ srcIp, path, visitCount, uas: uas.join("<br>"), firstTs, lastTs, timesList });
    }
